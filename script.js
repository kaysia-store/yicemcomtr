...
// (removed misplaced class terminator and constants block)
	...
	loadProducts() {
		// all.json dosyasından menü verilerini yükle
		console.log('Loading products from all.json...');
		...
		fetch(jsonPath)
			...
			.then(data => {
				console.log('all.json loaded successfully. Categories:', data.categories?.length || 0);
				...
				console.log(`Total products loaded from all.json: ${this.products.length}`);
				this.renderProducts();
			})
			.catch(error => {
				console.error('Error loading all.json:', error);
				this.products = [];
				this.renderProducts();
			});
	}

	// all.json -> dahili ürün yapısına dönüştür
	mapTranslatedMenuToProducts(data) {
		// all.json formatı: { restaurant: {...}, categories: [...] }
		console.log('mapTranslatedMenuToProducts called with data:', data);
		if (!data) {
			console.error('mapTranslatedMenuToProducts: data is null or undefined');
			return [];
		}
		if (!Array.isArray(data.categories)) {
			console.error('mapTranslatedMenuToProducts: data.categories is not an array', data.categories);
			return [];
		}
		console.log('mapTranslatedMenuToProducts: processing', data.categories.length, 'categories');
		const catMap = {
			pizza: 'pizzalar',
			toast: 'ayvalik-tostu',
		...
		for (const category of data.categories) {
			...
			for (const product of category.products) {
				...
				// all.json'da image alanı varsa onu kullan, yoksa PRODUCT_IMAGE_MAP veya getLocalImage
				const imagePath = product.image 
					? product.image 
					: ((typeof PRODUCT_IMAGE_MAP !== 'undefined' && PRODUCT_IMAGE_MAP[product.id])
